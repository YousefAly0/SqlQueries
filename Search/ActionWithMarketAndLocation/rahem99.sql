SELECT DISTINCT  prd.ep_id AS EpId, prd.ep_arabicName AS EpArabicName, prd.ep_englishName AS EpEnglishName, price.markpr_current_price AS Price, img.ep_img_path AS EpImageUrl, 
							...
							SELECT  mph_ep_id,mph_market_id,mph_updated_by , MAX(mph_updated_date) AS MaxDate
								 ...
								 ON  prd.ep_id	 = tm.mph_ep_id 
									        AND  tm.mph_updated_by = 23036
                            LEFT JOIN ph.ph_multiple_price_in_price_hunter multiplePrice ON multiplePrice.mph_ep_id = prd.ep_id	
								                 AND (multiplePrice.mph_created_by = 23036 OR multiplePrice.mph_updated_by = 23036)
...
WHERE 1=1 AND (prd.ep_disable_pr_inapp = 0 OR prd.ep_disable_pr_inapp IS NULL) AND market.market_id = 1 AND( assignedBrand.bas_assigned_user_id = 23036
			                                        OR assignedItemType.itas_assigned_user_id = 23036
			                                        OR assignedMarket.mas_assigned_w_user_id = 23036)
	AND prd.ep_id = 7623145 OR prd.ep_id = 7623255
